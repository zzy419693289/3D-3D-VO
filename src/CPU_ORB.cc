...
#include "CPU_ORB.h"
	...
	using namespace std;
	ORB_CPU::ORB_CPU(cv::Mat& Rawimage , int mfeatures, float mscaleFactor, int mlevels)
	{
		//double startime = (double)cv::getTickCount();//开始计时
		//cv:mat的创建只是创建了头部信息，但是数据信息完全可以通过后续赋值来获取。所以cv:mat完全可以动态初始化
		cv::cvtColor(Rawimage,rawimage,CV_RGBA2GRAY);	//4 channel RGB to 1 channel Gray;if CV_RGB2GRAY, means 3 channel to 1 channel
		nfeatures = mfeatures;
	...
	vector<cv::DMatch> ORB_CPU::MatchPic(ORB_CPU &orb1, ORB_CPU &orb2, bool cross, bool matchfilter)
	{
		//double startime = (double)cv::getTickCount();//开始计时
